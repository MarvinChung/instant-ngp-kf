...
struct CameraKeyframe {
	...
	CameraKeyframe(const Eigen::Vector4f &r, const Eigen::Vector3f &t, float sl, float sc, float fv, float df) : R(r), T(t), slice(sl), scale(sc), fov(fv), aperture_size(df) {}
	CameraKeyframe(Eigen::Matrix<float, 3, 4> m, float sl, float sc, float fv, float df) : slice(sl), scale(sc), fov(fv), aperture_size(df) { T=m.col(3); R=Eigen::Quaternionf(m.block<3,3>(0,0)).coeffs();  }
	CameraKeyframe operator*(float f) const { return {R*f,T*f,slice*f,scale*f,fov*f,aperture_size*f}; }
...
struct CameraPath {
	...
	int imgui(char path_filename_buf[128], float frame_milliseconds, Eigen::Matrix<float, 3, 4> &camera, float slice_plane_z, float scale, float fov, float aperture_size, float bounding_radius, const Eigen::Matrix<float, 3, 4> &first_xform);
...
#ifdef NGP_GUI
void add_debug_circle(ImDrawList* list, const Eigen::Matrix<float, 4, 4>& proj, Eigen::Vector3f a, float radius, uint32_t col=0xff4040ff, float thickness=1.f);
void visualize_map_point(ImDrawList* list, const Eigen::Matrix<float, 4, 4>& world2proj, Eigen::Vector3f map_point, uint32_t col=0xff4040ff);
